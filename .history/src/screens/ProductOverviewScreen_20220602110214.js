...
import {getHtmlTextOfProduct} from '../data/apis/product';
...
export const ProductOverviewScreen = ({navigation, route}) => {
  ...
  const fetchHtmlFile = async () => {
    const [result, message] = await getHtmlTextOfProduct(1);
    // const htmlText = await request.text();
    console.log(result);
    setHtmlText(result, message);
  };
  ...
  return (
    <CompatibleScroll style={styles.container}>
      ...
      <RenderHtml
        ...
        contentWidth={width}
        source={{
          uri: 'https://reactnative.dev/blog/2020/07/06/version-0.63',
        }}
      />
